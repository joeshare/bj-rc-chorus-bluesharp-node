    ...
    return new Promise((resolve, reject)=>{
        if(isError(res)){//报错了
            ...
            subject.rolebindResouce(mm.code, putobj, sessId, function (error, rolebind) {
                console.log("rolebindResouce ",mcfg.ID)
                console.log("rolebindResouce ",error)
                if(!error)
                {
                    if (mcfg.ID == OWNER_ROLE) {//给当前用户owner触角
                        console.log("rolebindResouce  userId",userId)
                        console.log("rolebindResouce  mm.code",mm.code)
                        console.log("rolebindResouce  sessId",sessId)
                        subject.addusetorole(userId, mm.code,sessId, function(aerror, adduserto) {
                            console.log("addusetorole ",aerror)
                            if(!aerror) {
...
function roleconfig2RequestArr(roleconfig,opt){
    ...
    let taskFun=(bindArg,callBack)=>{
        subject.createRole(bindArg.appCode, bindArg.subjectCode, bindArg.sessId, bindArg.projectName + '_' + bindArg.mcfg.name, bindArg.createUserId, function (error, mm) {
