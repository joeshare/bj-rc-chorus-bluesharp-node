...
data = data.replace(/sessionMode:('|")?\w*('|")?,/g,function (word){
    return 'sessionMode:"development",';//"development" cluster,
});
...
data = data.replace(/blueSharpRemoteHost:('|')?.+('|")?,/g,function (word){
    return 'blueSharpRemoteHost:"http://10.200.48.114",';//"development" cluster,
});
data = data.replace(/blueSharpRemotePort:('|")?.+('|")?,/g,function (word){
    return 'blueSharpRemotePort:"8080",';//"development" cluster,
});
